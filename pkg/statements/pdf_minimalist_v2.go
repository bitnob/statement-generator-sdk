...
// MinimalistPDFRenderV2 creates a clean, minimalist PDF with enhanced footer and logo support
func (s *Statement) MinimalistPDFRenderV2() ([]byte, error) {
	pdf := gofpdf.New("P", "mm", "A4", "")

	// Set margins for clean spacing
	pdf.SetMargins(20, 20, 20)
	pdf.SetAutoPageBreak(true, 30) // More space for footer
	...
	// Add Logo if available (SVG or image)
	logoHeight := 0.0
	if s.Input.Institution != nil {
		if s.Input.Institution.LogoSVG != "" {
			// For now, we'll add a placeholder for SVG
			// In production, you'd use a library to convert SVG to PDF
			pdf.SetFont("Helvetica", "B", 10)
			pdf.SetTextColor(darkGrey[0], darkGrey[1], darkGrey[2])
			pdf.CellFormat(0, 8, "[Logo]", "", 1, "L", false, 0, "")
			logoHeight = 10.0
		} else if len(s.Input.Institution.Logo) > 0 {
			// Add image logo
			imageType := "PNG"
			if len(s.Input.Institution.Logo) > 3 {
				// Check for JPEG magic bytes
				if s.Input.Institution.Logo[0] == 0xFF && s.Input.Institution.Logo[1] == 0xD8 {
					imageType = "JPG"
				}
			}
			...
			pdf.RegisterImageOptionsReader(imgName, gofpdf.ImageOptions{
				ImageType: imageType,
				ReadDpi:   true,
			}, bytes.NewReader(s.Input.Institution.Logo))

			// Place logo (max height 15mm)
			pdf.ImageOptions(imgName, 20, pdf.GetY(), 0, 15, false,
				gofpdf.ImageOptions{ImageType: imageType}, 0, "")
			logoHeight = 15.0
			pdf.SetY(pdf.GetY() + logoHeight)
		}
	}

	// Institution Header - Clean and minimal
	if s.Input.Institution != nil {
		// If we had a logo, add some space
		if logoHeight > 0 {
			pdf.Ln(3)
		}
	...
	// Contact Information
	if s.Input.Institution != nil && (s.Input.Institution.ContactPhone != "" || s.Input.Institution.ContactEmail != "") {
		pdf.SetFont("Helvetica", "B", 9)
		pdf.SetTextColor(black[0], black[1], black[2])
		pdf.CellFormat(0, 5, "Customer Service", "", 1, "L", false, 0, "")

		pdf.SetFont("Helvetica", "", 8)
		pdf.SetTextColor(darkGrey[0], darkGrey[1], darkGrey[2])

		if s.Input.Institution.ContactPhone != "" {
			pdf.CellFormat(0, 4, fmt.Sprintf("Phone: %s", s.Input.Institution.ContactPhone), "", 1, "L", false, 0, "")
		}
		if s.Input.Institution.ContactEmail != "" {
			pdf.CellFormat(0, 4, fmt.Sprintf("Email: %s", s.Input.Institution.ContactEmail), "", 1, "L", false, 0, "")
		}
		if s.Input.Institution.Website != "" {
			pdf.CellFormat(0, 4, fmt.Sprintf("Website: %s", s.Input.Institution.Website), "", 1, "L", false, 0, "")
		}
		pdf.Ln(3)
	}
	...
	footerText := "This is an official bank statement. Please review all transactions and report any discrepancies immediately."
	if s.Input.Institution != nil && s.Input.Institution.FooterText != "" {
		footerText = s.Input.Institution.FooterText
	}

	// Add account number reference in footer
	if s.Input.Institution != nil && (s.Input.Institution.ContactPhone != "" || s.Input.Institution.ContactEmail != "") {
		footerText += fmt.Sprintf(" Please reference your account number (%s) when contacting us.", s.Input.Account.Number)
	}
	...
	for i := 1; i <= pageCount; i++ {
		...
		pdf.SetFont("Helvetica", "", 8)
		pdf.SetTextColor(darkGrey[0], darkGrey[1], darkGrey[2])
		pdf.CellFormat(95, 5, fmt.Sprintf("Page %d of %d", i, pageCount), "", 0, "L", false, 0, "")

		// Generation timestamp - right aligned
		pdf.CellFormat(0, 5, fmt.Sprintf("Generated: %s", s.GeneratedAt.Format("Jan 2, 2006 at 3:04 PM MST")), "", 1, "R", false, 0, "")

		// Copyright or institution name
		pdf.SetY(-15)
		pdf.SetFont("Helvetica", "", 7)
		if s.Input.Institution != nil {
			pdf.CellFormat(0, 5, fmt.Sprintf("© %s. All rights reserved.", s.Input.Institution.Name), "", 0, "C", false, 0, "")
		}
	}
